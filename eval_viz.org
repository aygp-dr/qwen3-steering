#+TITLE: Terse/Verbose Evaluation Visualizations
#+AUTHOR: aygp-dr
#+PROPERTY: header-args:python :tangle eval_viz.py :mkdirp yes
#+STARTUP: overview

* Pipeline Overview

#+begin_src mermaid :file images/eval-pipeline.png :exports results
graph LR
    subgraph Data
        A[eval_output/terse_verbose_full.json]
    end
    subgraph "eval_viz.py (6 panels)"
        B[1. Descriptive KDE]
        C[2. Elbow + Silhouette]
        D[3. PCA Scatter]
        E[4. Confusion Matrix]
        F[5. Effect Size Heatmap]
        G[6. Dashboard]
    end
    subgraph Output
        H[eval_output/*.png]
    ...
    style G fill:#161b22,stroke:#f0883e,color:#c9d1d9

    OBSERVED_STATS["Replace with real data:\nterse μ=51, baseline μ=122, verbose μ=143\nvec_norm=19.59, n=10 (test run)"]
    style OBSERVED_STATS fill:#0d1117,stroke:#8b949e,color:#8b949e
#+end_src

The =OBSERVED_STATS= node is the substitution point: when the full 100-prompt
run completes, replace those values.  If ARI < 0.50 after substitution,
C-28 is refuted.
...
* Section 1 — Descriptive KDE

The word-count panel alone tells the story visually.  Median lines
make the per-direction center explicit.
...
Justifies k=3 without assuming it.  Elbow on inertia, silhouette score
across k=2..8.
...
Side-by-side: ground truth labels vs k-means predicted labels.
2-sigma confidence ellipses per group.
...
Raw and normalised side-by-side.  Burke's prediction: terse/baseline
will bleed, verbose will be clean (the screen pushes DOWN harder than UP).
...
* Section 5 — Effect size heatmap

Rank-biserial r for each feature × each direction pair.
...
Single composite figure with all five panels.  README-ready.
...
#+begin_example
C-28: Surface features are sufficient to recover the screening conditions
      without oracle labels.

Claim: K-means on (word_count, token_count, char_count) recovers the
       terse/baseline/verbose partition with ARI >= 0.50.

Refutation criterion: ARI < 0.50 on the full 100-prompt run means
       the terministic screens are NOT producing surface-level effects
       that match what the word-count numbers suggest.

Burke framing: if the screens are real, they select/deflect enough to
       show up in the simplest possible features.  If not, the "terse"
       screen is a prompt effect that doesn't survive to output length.
#+end_example
